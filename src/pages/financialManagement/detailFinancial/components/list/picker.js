...
export default class Picker extends Component {
	...
	addPicker = (isGetError) => {
		...
		} else if (new Date(`${startDate} ${startTime}`).getTime() > new Date(`${endDate} ${endTime}`).getTime()) {
			preList.error = true;
			console.log(666)
	...
	dataPickerChange = (date, index) => {
		...
		if (startDate && endDate && (new Date(`${startDate} ${startTime}`).getTime() > new Date(`${endDate} ${endTime}`).getTime()) || !startTime || !startDate || !endTime || !endDate) {
					...
					list.map((item, index) => {
						return (
							<li style={{ width: '660px' }} key={item.key} className={item.error ? styles.date_picker_error : styles.date_picker_ok}>
								<span>
									{/* <DatePicker 
									defaultValue={moment(item.startDate, 'YYYY-MM-DD')}
									className={styles.picker} 
									onChange={(_,date) => dataPickerChnage({startDate: date},index)}/> ~  <DatePicker
									defaultValue={moment(item.endDate, 'YYYY-MM-DD')}
									className={styles.picker}
									onChange={(_,date) => dataPickerChnage({endDate: date},index)}/> */}
									<RangePicker onChange={(_, [startDate, endDate]) => dataPickerChange({ startDate, endDate }, index)} />
								</span>
