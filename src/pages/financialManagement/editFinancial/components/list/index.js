  ...
  render() {
    ...
    return (
      <>
        ...
        {!this.props.isHide ? (this.props && (this.props.edit && (this.state.status == 1)) && (this.props.edit) || '') : (this.props && (this.props.edit && (this.state.status == 1)) && (this.props.edit) || this.props.cancel) }
      </>
    );
  }
}
...
class UnitInput extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      ...
      unit: this.props.selectDefaultValue,
      value: this.props.expiration_time || 1,
      ...(nextProps.option)
    })
    // console.log('emmm',this.state)
  }
  ...
  handleChange = (name = '', value = '') => {
    ...
    setTimeout(() => this.props.getUnitMsg({ value: { unit:['年','月','日'].indexOf(document.querySelector(".unitSelect .ant-select-selection-selected-value").title) + 1, value: +document.querySelector(".unitInput input").value }, status: '1' }),1000/60)
   this.setState(state)
  }
    ...
    return (
      <>
        ...
        ref="number"
          ...
          min={1}
          className="unitInput"
          defaultValue={this.props.expiration_time}
        ...
        <Select defaultValue={this.state.unit || '1'}
          className="unitSelect"
           style={{ width: this.props.selectWidth || 'auto',marginLeft: '8px' }}
...
class CreateFinancial extends Component {
    ...
    list = {
      temp1: [
        {
          ...
          label: '套餐名称',
          option: {
            initialValue: isEditPage ? (data.package_name ? data.package_name : '套餐1') : '套餐1',
            rules: [{ required: true, message: '请输入套餐名称', ...(isEditPage ? { initialValue: data.package_name ? data.package_name : '套餐1' } : {}) }],
          },
          content:
            <AstInput
              onChange={e => {
            ...
            <RadioGroup
              ...
              cancel={<><InputNumber
                ...
                style={{ width: '176px',marginRight: '10px' }}
                  name="limit_number"
                placeholder="请输入场次数"
                defaultValue={isEditPage ? (data.limit_number || 0) : 1} />场</>}
            />
        },
        {
        ...
        },
        {
          ...
          label: '有效期限',
          option: {
            ...(isEditPage ? { initialValue: { status: data.is_expiration ? '1' : '0', value: { value: `${data.expiration_time}`, unit: data.expiration_time_unit } } } : { initialValue: { status: 0, value: { value: 1, unit: 1 } } }),
          ...
          content: <RadioGroup
            ...
            edit={<UnitInput
              ...
              expiration_time={data.expiration_time || 1}
              // option={isEditPage ? { value: `${data.expiration_time || '55'}`, unit: data.expiration_time_unit || '1' } : {}}
      ...
      temp2: [
        ...
        },
        {
          ...
          label: '可开播时间',
          option: {
            initialValue: {status: isEditPage ? +data.is_time_limit : '0',value: data.time_limit || ''},
          ...
          content: <RadioGroup
            ...
            edit={<Picker
              name="time_limit"
              pickTime={JSON.parse(data.time_limit || '[]')}
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if(is_number_limit == 1 && !document.querySelector('input[name=limit_number').value){
        canSend = false;
        this.setError("limit_number",'限制场次数不合法!')
      }
      if(document.querySelector('input[name=suggested_price]').value == ''){
        canSend = false;
        this.setError('suggested_price','套餐参考价不可以为空!')
      }
      if(document.querySelector('input[name=single_num]').value == ''){
        canSend = false;
        this.setError("selling_price",'售价不可为空!')
      }
      if(is_time_limit == 1 && (time_limit == '' || is_time_limit == '[]')){
        canSend = false;
      ...
      if(canSend){
        this.props.dispatch({
              type: 'financial/updateFinancial',
  ...
  filterDate = (data = {}) => {
    ...
    let financialObj = Object.assign({ ...data }, {
      ...
      time_limit: typeof time_limit.value == 'string' ? time_limit.value : JSON.stringify((time_limit.value || []).map(item => {
  ...
  handleCollectInputDate = (name, data) => {
    ...
    if(name == 'selling_price' || name == 'suggested_price' || name == 'single_num' || name == 'package_name' || name == 'playback_store' || name == 'extent_live_time'){
      ...
      <div className={style.financial_create}>
        <Form labelCol={{ span: 5 }} wrapperCol={{ span: 12 }}>
          ...
          <Form.Item wrapperCol={{ span: 12, offset: 2 }}>
            <Button type="primary" onClick={this.handleSubmit} style={{marginLeft: 16}}>
              修改
            </Button>
